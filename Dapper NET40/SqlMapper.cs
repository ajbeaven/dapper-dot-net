...
this IDbConnection cnn, string sql, bool handleAbstractTypes, object param, IDbTransaction transaction, int? commandTimeout, CommandType? commandType
		...
		{
			return type.IsSealed == false && type.GetProperty("Discriminator") != null;
		}

		private static Type GetDerivedType(Type type, IDataReader reader, int startBound, int length)
		{
			var invalidTypeException = new InvalidOperationException("Cannot create instance of abstract class " + type.FullName + ". To allow dapper to map to a derived type, add a Discriminator field that stores the name of the derived type");;
			...
			for (var i = startBound; i < (startBound + length); i++)
			{
				if (reader.GetName(i) == "Discriminator")
					discriminator = reader[i].ToString();
			}

			// HACK: if the type we're mapping to has a Discriminator field but the DataReader doesn't have any data for that type (ie. left join that doesn't match)
			// there's no way to determine the actual derived type. Because it's just going to map to a null instance anyway, just return any non-abstract derived type
			if (String.IsNullOrEmpty(discriminator))
			{
				if (!type.IsAbstract)
					return type;

				// HACK: find the first non abstract derived type and return that
				var firstNonAbstractDerivedType = assembly.GetTypes().FirstOrDefault(t => !t.IsAbstract && t.IsSubclassOf(type));
				if (firstNonAbstractDerivedType != null)
					return firstNonAbstractDerivedType;

				// this class has no derived types to use
				throw invalidTypeException;
			}
...
Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
...
)
		{
			...
			type = GetDerivedType(type, reader, startBound, length);
...
#pragma warning restore 618
		...
		public partial class GridReader : IDisposable
		{
            ...
            {
				var data = ReadImpl<dynamic>(typeof(DapperRow));
			...
			{
				if (type == null) throw new ArgumentNullException("type");
				var data = ReadImpl<object>(type);
				return buffered ? data.ToList() : data;
			}

			private IEnumerable<T> ReadImpl<T>(Type type)
			{
				...
				// HACK: old implementation calls GetColumnHash before we read the IDataReader, which fails if there is no data
				if (consumed) throw new InvalidOperationException("Query results must be consumed in the correct order, and each result can only be consumed once");
				var typedIdentity = identity.ForGrid(type, gridIndex);
				CacheInfo info = GetCacheInfo(typedIdentity, null, addToCache);
				bool canBeDerived = CanTypeBeDerived(type);
				consumed = true;
